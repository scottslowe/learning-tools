...
By default, the Docker image name is `huangzhaorongit/<service>` where `<service>` is `greeting`, `name` or `webapp`. The image can be created in your repo:
...
. Configure secret for Docker registry in the cluster:
  ...
  kubectl create secret \
    ...
    --docker-username=huangzhaorongit \
...
. `huangzhaorongit/xray:us-west-2` Docker image is already available on Docker Hub. Optionally, you may build the image:

  cd config/xray
  docker build -t huangzhaorongit/xray:latest .
  docker image push huangzhaorongit/xray:us-west-2
