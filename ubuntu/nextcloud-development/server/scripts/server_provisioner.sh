...
#nginx nginx
#envsubst \$NGINX_HOST},\$NGINX_ROOT,\$ERROR_PATH < "/vagrant/server/virtualhosts/nginx.conf.template" > "/etc/nginx/sites-enabled/default.conf"
...
#sudo openssl req -x509 -nodes -days 365 -newkey rsa:4096 -keyout /etc/ssl/nginx/${NGINX_HOST}.key -out /etc/ssl/nginx/${NGINX_HOST}.crt
#sudo openssl dhparam -out /etc/ssl/nginx/${NGINX_HOST}.pem 4096
