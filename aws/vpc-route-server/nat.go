...
func buildNat(ctx *pulumi.Context, z InfraDetails) (InfraDetails, error) {
	...
	z.natPublicIp = natInstance.PublicIp
	...
	if err != nil {
		log.Printf("error creating routes for private subnets: %s", err.Error())
		return z, err
	}

	// Uncomment the following lines for additional outputs that may be useful for troubleshooting/diagnostics
	// ctx.Export("natPublicIp", z.natPublicIp)
	// ctx.Export("natPrivateIp", natInstance.PrivateIp)
	// ctx.Export("natInstanceId", natInstance.ID())
	// ctx.Export("natAmiId", pulumi.String(natAmi.Id))
