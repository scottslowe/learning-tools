...
INSTANCEID=$(aws ec2 run-instances --image-id $AMIID --key-name hzrkey --instance-type t2.micro --security-group-ids $SGID --subnet-id $SUBNETID --query "Instances[0].InstanceId" --output text)
...
echo "ssh -i hzrkey.pem ec2-user@$PUBLICNAME"
